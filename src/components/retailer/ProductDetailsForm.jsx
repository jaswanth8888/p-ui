...
import { InputLabel, TextField, Typography, Table } from "@material-ui/core"
...
class ProductDetailsForm extends Component {
  ...
  handleSubmit(e) {
    ...
    const { productBasePrice } = this.state
    const { productVendor } = this.state
    const { productQuantity } = this.state
    const { productImages } = this.state
    const { productDescription } = this.state
    const { productCategory } = this.state
    const { AssignedClusterName } = this.state
    const { AssignedClusterQuantity } = this.state
    const { AssignedClusterPrice } = this.state
    const { AssignedClusterProfitPercentage } = this.state
    const { MinimunSellingPrice } = this.state
        ...
        <div className="joint-form" style={{ width: "850px" }}>
          ...
          <form className="{classes.form}" noValidate>
            <FormControl style={{ top: 265, alignSelf: "center" }}>
              <InputLabel id="demo-simple-select-label" style={{ left: 250 }}>
                Cluster Name
              </InputLabel>
              <Select
          ...
          <div className="store-requirement">
            ...
            {this.state.AssignedClusterQuantity >=
              this.state.productQuantity && (
              <div style={{ display: "flex", color: "#ffc107" }}>
                <CheckIcon style={{ paddingRight: "5px", marginTop: "-2px" }} />
                <Typography variant="subtitle2" gutterBottom>
                  Assigned quantity for the cluster should be less than the
                  quantity available
                </Typography>
              </div>
            )}
